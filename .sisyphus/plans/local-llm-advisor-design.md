...
> **What**: A self-contained macOS (Apple Silicon) desktop app that profiles your hardware, recommends open LLMs that actually fit in memory, downloads them from HuggingFace, and serves an OpenAI-compatible inference endpoint backed by a bundled llama.cpp `llama-server`.
>
> **Stack**: Tauri 2.x · Rust core · web UI · bundled llama-server sidecar (MIT)
>
...
### Product Goals
- **G1** — Accurate Apple Silicon hardware profiling (CPU cores, unified RAM, GPU, disk free)
...
| Excluded | Rationale |
|---|---|
| Windows / Linux / Intel Mac | v1 is Apple Silicon only; graceful "unsupported" screen elsewhere |
...
| llama-server | Internal (process) | Bundled MIT sidecar doing actual inference over Metal |

### Assumptions & Constraints
- Target: Apple Silicon Macs (M1–M4). Unified memory ⇒ **there is no dedicated VRAM**; the usable budget derives from total RAM × Metal working-set ceiling.
- `MTLDevice.recommendedMaxWorkingSetSize` queried at **runtime** (small native shim); hardcoded 75%-of-RAM is fallback only.
- llama-server is pinned to a fixed upstream release + sha256 (supply-chain guard).
...
memory_needed(ServeConfig) =
  ...
  + runtime_overhead         // ≈ 0.7 GB Metal floor
budget = min(metal_working_set_bytes, total_ram_bytes)
verdict: needed ≤ budget → FIT; else compute max_context_that_fits by solving ctx down
speed_estimate ∝ memory_bandwidth / active_params (MoE: VRAM↔TOTAL params, speed↔ACTIVE params)
```
...
|----|----------|--------------|---------|-------|
| UC1 | View Hardware Profile | End User | Dashboard shows chip name, core counts, unified RAM, Metal working-set ceiling, disk free | 1 |
| UC2 | Detect / Refresh Specs | End User | Probe via sysinfo + system_profiler + Metal shim; cache profile with timestamp; re-run on demand | 1 |
...
- `metal_working_set_bytes` comes from the runtime Metal query with the 75% heuristic only as fallback.
- MoE entries fill `active_params_b`; VRAM math uses total params, speed math uses active.
...
sequenceDiagram
    ...
    par CPU / RAM / Disk
        HW->>SI: refresh_all()
        SI-->>HW: cores, total_ram_bytes, disks
    and Chip / GPU identity
        HW->>SP: SPDisplaysDataType -xml + SPHardwareDataType
        SP-->>HW: chip_name, gpu_name, gpu_core_count
    end
    HW->>MT: recommendedMaxWorkingSetSize()
    alt Metal query succeeds
        MT-->>HW: working_set_bytes (runtime truth)
    else query fails
        MT-->>HW: error → fallback = 0.75 × total_ram
    end
    HW->>HW: budget = min(working_set, total_ram)
    HW-->>UI: HardwareProfile (cached with timestamp)
    UI-->>U: dashboard renders specs + memory budget bar
```
...
sequenceDiagram
    ...
    alt available < est_total_bytes
        ...
        SM->>LS: spawn sidecar -m model.gguf --port P -c ctx -np slots -ctk/-ctv -ngl all
...
```mermaid
flowchart TD
    A(["Launch app"]) --> B["Hardware profiled<br/>budget bar shown"]
    B --> C{"RAM >= 16 GB?"}
...
| ADR-7 | **Runtime Metal working-set query** | `recommendedMaxWorkingSetSize` varies by chip/OS; 75% heuristic only as fallback | Hardcoded ratio |
...
|---|----------|-----------|--------------|
| E1 | Non-Apple-Silicon Mac (Intel / non-macOS) | arch/chip probe at startup | Full-screen "Unsupported platform" notice; no features |
