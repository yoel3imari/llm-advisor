...
/// Estimate tokens-per-second throughput based on memory bandwidth and active parameter count.
pub fn estimate_speed_tps(
    entry: &CatalogEntry,
    _profile: &HardwareProfile,
    recommended_gpu_layers: u32,
) -> f32 {
    let active_params = entry
        .active_params_b
        .unwrap_or(entry.params_billions)
        .max(0.1);

    let host_ddr_bandwidth_gbps = 40.0f32;
    let dgpu_gddr_bandwidth_gbps = 220.0f32;

    let effective_bandwidth = if entry.n_layers > 0 && recommended_gpu_layers > 0 {
        let gpu_ratio = (recommended_gpu_layers as f32) / (entry.n_layers as f32);
        let cpu_ratio = 1.0 - gpu_ratio;
        (gpu_ratio * dgpu_gddr_bandwidth_gbps) + (cpu_ratio * host_ddr_bandwidth_gbps)
    } else {
        host_ddr_bandwidth_gbps
    };

    let efficiency_factor = 0.65f32;
    let raw_tps = (effective_bandwidth * efficiency_factor) / active_params;
    raw_tps.clamp(0.5, 180.0)
}
